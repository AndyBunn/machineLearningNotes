...
```
## Rows: 1600 Columns: 65
## ── Column specification ────────────────────────────────────────────────────────
## Delimiter: ","
## chr  (1): X1
## dbl (64): X2, X3, X4, X5, X6, X7, X8, X9, X10, X11, X12, X13, X14, X15, X16,...
## 
## ℹ Use `spec()` to retrieve the full column specification for this data.
## ℹ Specify the column types or set `show_col_types = FALSE` to quiet this message.
```
...
| | |
|:---:|:---:|
| ![](assets/Acer_circinatum/Acer_Circinatum_01.ab.jpg){width=45%} | ![](assets/Acer_circinatum/Acer_Circinatum_02.ab.jpg){width=45%} |
| ![](assets/Acer_circinatum/Acer_Circinatum_03.ab.jpg){width=45%} | ![](assets/Acer_circinatum/Acer_Circinatum_04.ab.jpg){width=45%} |
...
### Visualize the shape of Acer circinatum
...
``` r
leafShape %>% filter(species == "Acer circinatum") %>%
  mutate(sample = row_number()) %>%
  pivot_longer(cols = shapeValue1:shapeValue64, names_to = "position", values_to = "value") %>%
  mutate(position = as.integer(str_remove(position, "shapeValue"))) %>%
  filter(sample %in% 1:4) %>%
  ggplot(aes(x = position, y = value, color = as.factor(sample))) +
  geom_line(alpha = 0.6) +
...
<img src="09iveJustSeenAFace_files/figure-html/unnamed-chunk-28-1.png" alt="" width="672" />
...
The data are already in the right orientation -- one leaf per row, 64 shape values as columns. No transpose needed. Run `prcomp` with `center = TRUE` on just the descriptor columns (not the species label). Plot cumulative variance explained.
...
Plot the first several eigenvectors as line plots -- the same way you'd plot the shape descriptors themselves. These are the "eigenleaves." What does the first eigenvector look like? What might it represent? Think about what kinds of variation across leaf outlines PCA would pull out first. Overall size? Elongation? Symmetry? Lobedness? Unlike the eigenfaces, which are haunting and uninterpretable, the eigenleaves might actually tell you something biological.


``` r
# your eigenleaf visualization here
```

### Map the species

Average the PCA scores for each species across its 16 samples and plot them in the space of the first two PCs, labeled by species number. Which species are close together? Which are far apart? If you happen to know what some of the species are (the data set includes taxa like *Quercus*, *Salix*, *Acer*, and many others -- [see the paper](https://doi.org/10.24432/C5RG76) for the full list), do the groupings make any botanical sense?
...
Include a short reflection. What worked? What surprised you? If accuracy is lower than you hoped, what would you try next -- more PCs, a different descriptor (margin? texture?), a different distance metric?
