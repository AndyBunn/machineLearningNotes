...
thres95 <- min(which(cumVar > 0.99))
thres95
# typically around 20-30 PCs -- much more compact than the ~120 for faces
...
as_tibble(t(eigenvectors[, 1:9])) %>%
  ...
  labs(x = "Descriptor position", y = "Eigenvector value",
       title = "The first 9 eigenleaves")
...
ggplot(avgScores, aes(x = PC1, y = PC2, label = species)) +
  geom_point() +
...
thres95train <- min(which(cumVarTrain > 0.95))

evTrain     <- leafPCAtrain$rotation[, 1:thres95train]
evVals      <- eigenvaluesTrain[1:thres95train]
scoresTrain <- leafPCAtrain$x[, 1:thres95train]
